...
function owner_details_name(employee_name, email_id) {
  return new Promise(async function(resolve, reject) {
    ...
    // -----
    var res = await mysqlConnection.query_execute(query.getownerdetails, param);
...
function hr_details_name(email_id) {
  return new Promise(async function(resolve, reject) {
    ...
    // -----
    var res = await mysqlConnection.query_execute(query.gethrdetails, param);
    console.log("response", res);
    if (res.data.errno) {
      return reject({
        status: 400,
...
function add_admin(select_query) {
  return new Promise(async function(resolve, reject) {
    ...
    // -----
    var res = await mysqlConnection.insert_query(query.registeradmin, param);
    console.log("response", res);
    if (res.data.errno) {
      return reject({
        status: 400,
    ...
    await mysqlConnection
      ...
      .then(function(result, err) {
        if (err) {
          logger.fatal(
            err,
            "db error while intering the data into the citizen table while registering"
          );
          return resolve({
            status: 400,
            err: err
