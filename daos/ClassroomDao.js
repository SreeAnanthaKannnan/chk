    ...
    return new Promise( async function (resolve,reject){
       ...
       start_time=params[0][7]
        for(i=0;i<insert_count;i++){
                ...
                var res= await mysqlConnection.query_execute("SELECT ADDTIME( '"+ duration +"' , '" +start_time+ "') as started_time", [])
...
function Classroom_select(param1,param2,param3) {
    ...
    return new Promise( async function (resolve,reject){
       ...
       await con.query("SELECT classroom_id FROM Classroom where (classroom_id ='" + param1 + "')AND (available_date ='" + param2 + "')AND (time_slot = '" + param3 + "')" , (err, result) => {
            if(!result) { 
                //  console.log(result,"achieved")
                console.log("something",err)
                return resolve({ status: 400, err : err })
...
function Availability(classroom_id,available_date,start_time,end_time) {
    ...
    return new Promise( async function (resolve,reject){
        ...
        // params =[params]
        await con.query("SELECT * FROM Classroom where available_date ='" + available_date + "' and start_time ='" + start_time + "' and end_time = '" + end_time + "' and classroom_id = '" + classroom_id + "' " , (err, result) => {
            if(err) { 
            //  console.log(result,"achieved")
            console.log("something",err)
            return resolve({ status: 400, err: err })
...
function Availability_Date(no_of_seats_selected,trainer_id,course_id) {
    ...
    return new Promise( async function (resolve,reject){
        //  console.log("hiiiii",params)
        // params =[params]
        var res1=await mysqlConnection.query_execute("SELECT distinct available_date FROM Classroom where number_of_available_seats >=?and trainer_id=? and course_id =?",["0",trainer_id,course_id])
       ...
       var result1 ="";
        ...
        var value1 = date+1;
       let result2 = value1.split(' ').slice(0, 4).join(' ')
...
//===================================================================================================//
function time_slots_lists(available_date) {
    ...
    return new Promise( async function (resolve,reject){
        ...
        var obj ={};
        var obj_id =""
        var res=await mysqlConnection.query_execute("SELECT classroom_id FROM Classroom where available_date =?",[available_date])
        console.log(res,"res")
       const categories = [...new Set(res.data.map(bill => bill.classroom_id))]
       console.log(categories,"test")
       for(i=0;i<categories.length;i++){
        var res1=await mysqlConnection.query_execute("SELECT start_time,end_time,number_of_available_seats FROM Classroom where classroom_id =?",[categories[i]])
            console.log(res1.data.length,"kavitha")
            obj_id = categories[i]
            
            for(j=0;j<res1.data.length;j++){
         var res_start_time=await mysqlConnection.query_execute("SELECT TIME_FORMAT(?, '%h:%i %p') as start_time",[res1.data[j].start_time]);
         var res_end_time=await mysqlConnection.query_execute("SELECT TIME_FORMAT(?, '%h:%i %p') as end_time",[res1.data[j].end_time]);
        let no_of_available_seats = res1.data[j].number_of_available_seats; 
        console.log(res_start_time,"start_time")
         object_value = {classroom_id : categories[i],no_of_available_seats : no_of_available_seats,start_time:res_start_time.data[0].start_time , end_time :res_end_time.data[0].end_time}
...
function Seat_count(param) {
    ...
    return new Promise( async function (resolve,reject){
       ...
       await con.query("SELECT SUM(number_of_available_seats) as no_of_available_seats FROM Classroom where available_date ='" + param + "' and status ='" + "not filled" + "'" , (err, result) => {
            if(!result) { 
                //  console.log(result,"achieved")
                console.log("something",err)
                return resolve({ status: 400, err : err })
...
function Classroom_num(classroom_id,language) {
    ...
    return new Promise( async function (resolve,reject){
        //  param = moment(param).format("YYYY-MM-DD")
         ...
         if(language =="en"){
            var res1= await mysqlConnection.query_execute("SELECT * FROM Classroom where classroom_id=?",[classroom_id])
...
let sql = "SELECT TIMEDIFF( '"+ end_time +"' , '" +start_time+ "') as starting_time"
    ...
    //==============================================================================================//
    function bulk_booking1(course_id,no_of_seats_selected,language,Emirates_ID,Company_Trade_Lincense_No,scheduling_date,amount) {
        ...
        return new Promise( async function (resolve,reject){
          ...
          var res_value= await mysqlConnection.query_execute("select * from Classroom where number_of_available_seats <> 0 and course_id=? order by available_date asc",[course_id])
                   ...
                   if(classrooms[0].number_of_available_seats>=no_of_seats_selected){
                    ...
                    for(j=0;no_of_seats_selected>0;j++){
                      ...
                      //   if(param[0][1]<=result[j].no_available_seats){value.push(result[j].classroom_id)}
                        ...
                        obj ={classroom_id:classrooms[j].classroom_id,
                        ...
                        if(j!=0){
                         ...
                         if(no_of_seats_selected<classrooms[j-1].number_of_available_seats){
                            obj2 ={classroom_id:classrooms[j-1].classroom_id,no_available_seats:no_of_seats_selected}
      ...
      //=====================================================================================//
      function bulk_booking(course_id,no_of_seats_selected,language,Emirates_ID,Company_Trade_Lincense_No,scheduling_date,amount) {
          ...
          var res_value= await mysqlConnection.query_execute("select * from Classroom where number_of_available_seats <> 0 and course_id=? order by available_date asc",[course_id])
                   ...
                   for(employee of Emirates_ID){
                    ...
                    // console.log("resvalue=========>res2",res2)
                     ...
                     console.log("employee "+employee + " allocated to seat " + seats[counter++])
                     console.log("hi")
