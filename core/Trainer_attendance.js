...
async function trainer_attendance(trainer_data, token, language) {
    ...
    if (verifytoken.status == 402) {
      ...
      if (select_query.message.length == 0) {
        ...
        if (select_query.message != 0) {
          ...
          console.log(
            "core_select_query_scheduler_length===>>",
            select_query_scheduler.message.data.length
          );
          ...
          if (select_query_scheduler.message.data.length != 0) {
            for (i = 0; i < select_query_scheduler.message.data.length; i++) {
              ...
              var date =
                select_query_scheduler.message.data[i].scheduling_date + 1;
              var newdate = moment(date).format("YYYY/MM/DD");
              select_query_scheduler.message.data[i].scheduling_date = newdate;

              var date1 =
                select_query_scheduler.message.data[i].Scheduled_date + 1;
              var schedulleddate = moment(date1).format("YYYY/MM/DD");
              select_query_scheduler.message.data[
                i
              ].scheduled_date = schedulleddate;
              console.log("schedulleddate=======>", schedulleddate);
            }
            console.log("core  select_query_scheduler", select_query_scheduler);
            ...
            for (i = 0; i < select_query_scheduler.message.data.length; i++) {
              ...
              var start_time =
                select_query_scheduler.message.data[i].start_time;
...
async function trainer_date_select(Trainer_selecting_date, token, language) {
  return new Promise(async (resolve, reject) => {
...
async function trainer_attendance_list(Employee_attendance, token, language) {
  return new Promise(async (resolve, reject) => {
...
async function trainer_attendance_absent_list(Employee_attendance, token) {
  return new Promise(async (resolve, reject) => {
    var verifytoken = await checktoken.checkToken(token);
    console.log(verifytoken);
    if (verifytoken.status == 402) {
